        ...
        <div class="flex items-center justify-between px-2 py-2">
            <div class="flex flex-col min-w-0 pr-2">
                <div class="flex items-center gap-2">
                    <span class="text-[14px] font-bold text-white truncate">{{ auth()->user()->name ?? 'Quản trị viên' }}</span>
                    @if(auth()->check() && auth()->user()->role)
                        <span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-primary/20 text-primary uppercase shrink-0">{{ auth()->user()->role->name }}</span>
                    @endif
                </div>
                <span class="text-[12px] text-slate-500 truncate mt-0.5">{{ auth()->user()->email ?? 'admin@example.com' }}</span>
            </div>
            <form action="{{ route('logout') }}" method="POST" class="inline" 
                  data-confirm-submit 
                  data-confirm-title="Xác nhận đăng xuất" 
                  data-confirm-message="Bạn có chắc chắn muốn đăng xuất khỏi hệ thống không?"
                  data-confirm-accept-html="<i class='pi pi-sign-out text-[12px]'></i> Đăng xuất">
                @csrf
                <button type="submit" class="text-slate-500 hover:text-white transition-colors p-1" title="Đăng xuất">
